/*
 * Programacion Interactiva
 * Autor: Julian Andres Orejuela Erazo - 1541304 
 * Autor: Daniel Felipe Velez Cuaical - 1924306
 * Mini proyecto 3: Juego de poker clasico.
 */
package classicPoker;

/**
 * 
 */
public class TimeControl {
    public static final int LOADING = 100;

    public static final int apuestaInicial = 1000;
    public static final int rondaDeApuesta = 1000;
    public static final int jugadorPensadoSuApuesta = 500;
    public static final int jugadorHaTomadoDecision = 500;
    public static final int tiempoDeDescarte = 3000;

    /*
    public static final int APUESTA_INICIAL = 1000;
    public static final int RONDA_DE_APUESTA = 1000;
    public static final int JUGADOR_PENSANDO_APUESTA = 500;
    public static final int JUGADOR_TOMA_DECISION = 500;
    public static final int TIEMPO_DE_DESCARTE = 3000;
    */

}
